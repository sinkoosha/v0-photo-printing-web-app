  ...
  return (
    <section className="mx-auto w-full max-w-[1200px] px-4 pb-12 md:px-6">
      <div className="mb-6 flex items-center justify-between gap-4 md:mb-8">
        <h3 className="text-xl font-semibold text-[#3B2F2A] md:text-2xl">آخرین مقالات بلاگ</h3>
        <Link
          href="#"
          className="hidden rounded-full border border-[#E9E1DA] px-4 py-2 text-sm text-[#5A4A40] transition hover:border-[#C78555] hover:text-[#3B2F2A] md:inline-flex"
        >
          مشاهده همه
        </Link>
      </div>
      <div className="grid snap-x snap-mandatory grid-flow-col gap-4 overflow-x-auto pb-4 auto-cols-[minmax(240px,1fr)] sm:grid-flow-row sm:grid-cols-2 sm:gap-6 sm:overflow-visible sm:pb-0 lg:grid-cols-4">
        {posts.map((post) => (
          <article
            key={post.title}
            className="group flex h-full snap-center flex-col overflow-hidden rounded-[28px] border border-[#E9E1DA] bg-white/90 shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
          >
            ...
            <div className="flex flex-1 flex-col items-end gap-3 px-4 py-4 text-right">
              <h4 className="text-sm font-semibold text-[#3B2F2A]">{post.title}</h4>
              <Link href="#" className="text-xs font-medium text-[#C78555] transition hover:text-[#B87445]">
                مطالعه بیشتر
              </Link>
            </div>
          </article>
