    ...
    <div className="flex min-h-screen flex-col bg-[#F6EFE9]">
      ...
      <main className="flex-1 pb-24">
        <div className="mx-auto w-full max-w-[1200px] px-4 md:px-6">
          <nav className="flex items-center gap-2 text-sm text-[#9C8C7F]" aria-label="مسیر صفحه">
          ...
          <section aria-label="گالری تصاویر محصول" className="mt-10">
            <h2 className="mb-4 text-lg font-semibold text-[#3B2F2A]">نمای نزدیک از چاپ‌ها</h2>
            <div className="flex gap-4 overflow-x-auto rounded-3xl border border-[#E9E1DA] bg-white p-4">
              {galleryImages.map((image) => (
                <div
                  key={image.alt}
                  className="relative h-32 w-48 shrink-0 overflow-hidden rounded-2xl border border-[#E9E1DA]/70 shadow-sm"
                >
            ...
            {highlightFeatures.map((feature) => (
              ...
              >
                ...
                <div className="space-y-2">
                  <h3 className="text-lg font-semibold text-[#3B2F2A]">{feature.title}</h3>
                  <p className="text-sm leading-7 text-[#5A4A40]">{feature.description}</p>
                </div>
              </article>
            ))}
          </section>
          ...
          <section className="mt-12 rounded-3xl border border-[#E9E1DA] bg-white p-6 shadow-sm">
            <h2 className="text-lg font-semibold text-[#3B2F2A]">جزئیات فنی محصول</h2>
            <dl className="mt-6 grid gap-x-6 gap-y-4 sm:grid-cols-2">
              {productSpecs.map((spec) => (
                <div key={spec.label} className="flex flex-col gap-1 rounded-2xl bg-[#F9F3ED] p-4">
                  <dt className="text-xs font-medium uppercase tracking-[0.2em] text-[#C78555]">{spec.label}</dt>
          ...
          <section className="mt-12 grid gap-6 lg:grid-cols-[0.65fr_0.35fr]">
            <div className="space-y-6 rounded-3xl border border-[#E9E1DA] bg-white p-6 shadow-sm">
              <h2 className="text-lg font-semibold text-[#3B2F2A]">آماده‌سازی و ارسال</h2>
            ...
            <aside className="space-y-4 rounded-3xl border border-[#E9E1DA] bg-white p-6 shadow-sm">
              <h3 className="text-base font-semibold text-[#3B2F2A]">سرویس مشتریان</h3>
          ...
          <section className="mt-12 rounded-3xl border border-[#E9E1DA] bg-white p-6 shadow-sm">
            <h2 className="text-lg font-semibold text-[#3B2F2A]">سؤالات متداول</h2>
            <div className="mt-6 space-y-4">
              {faqs.map((faq) => (
                <article key={faq.question} className="rounded-2xl border border-[#E9E1DA] bg-[#F9F3ED] p-4">
                  <h3 className="text-sm font-semibold text-[#3B2F2A]">{faq.question}</h3>
                  <p className="mt-2 text-sm leading-7 text-[#5A4A40]">{faq.answer}</p>
                </article>
              ))}
            </div>
          </section>
          ...
          <section className="mt-12 grid gap-6 md:grid-cols-3">
            {serviceHighlights.map((item) => (
              <div key={item.title} className="rounded-3xl border border-[#E9E1DA] bg-white p-6 shadow-sm">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#F1E4D7] text-[#C78555]">
                  <item.icon className="h-7 w-7" />
                </div>
                <h3 className="mt-4 text-base font-semibold text-[#3B2F2A]">{item.title}</h3>
                <p className="mt-2 text-sm leading-7 text-[#5A4A40]">{item.description}</p>
              </div>
            ))}
          </section>

          <section className="mt-12 rounded-3xl border border-[#E9E1DA] bg-white p-6 shadow-sm">
            <h2 className="text-lg font-semibold text-[#3B2F2A]">گواهی‌ها و مجوزها</h2>
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              {trustBadges.map((badge) => (
                ...
                >
                  ...
                  <p className="text-sm font-semibold text-[#3B2F2A]">{badge.label}</p>
...
function ProductSummary() {
  return (
    <section className="space-y-6 rounded-3xl border border-[#E9E1DA] bg-white p-6 shadow-sm" aria-labelledby="product-heading">
      ...
      <div className="rounded-3xl bg-[#F9F3ED] p-5">
        <div className="flex flex-col gap-3 text-sm">
          ...
          <div className="flex items-baseline gap-3 text-[#3B2F2A]">
            ...
            <span className="rounded-full bg-white px-3 py-1 text-xs text-[#C78555]">پکیج ۲۰ عددی</span>
          </div>
          <div className="flex items-center gap-3 text-xs text-[#9C8C7F]">
            <span className="line-through">۳۹۹٬۰۰۰ تومان</span>
            <span className="font-medium text-[#C78555]">۱۰٪ تخفیف خرید آنلاین</span>
          </div>
        </div>
      </div>
      ...
      <section aria-label="انتخاب تیراژ" className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-[#9C8C7F]">بیشتر بخرید، بیشتر تخفیف بگیرید</span>
        </div>
          ...
          {bundleOptions.map((option, index) => (
            <button
              ...
              className={`flex flex-col items-start rounded-2xl border px-4 py-3 text-right transition hover:border-[#C78555] hover:bg-[#F9F3ED] ${
                index === 1 ? "border-[#C78555] bg-[#F9F3ED] shadow-sm" : "border-[#E9E1DA] bg-white"
              }`}
            >
              <span className="text-sm font-semibold text-[#3B2F2A]">{option.label}</span>
              <span className="text-xs text-[#5A4A40]">{option.price} تومان</span>
      ...
      <section aria-label="افزودنی‌ها" className="space-y-3">
        <h2 className="text-sm font-semibold text-[#3B2F2A]">افزودنی‌های چاپ</h2>
        <ul className="space-y-3">
          {printAddons.map((addon) => (
            <li key={addon.title} className="flex items-center justify-between rounded-2xl bg-[#F9F3ED] px-4 py-3 text-sm text-[#3B2F2A]">
              <div>
                <p className="font-medium">{addon.title}</p>
                <p className="text-xs text-[#5A4A40]">{addon.description}</p>
              </div>
              <span className="text-xs font-semibold text-[#C78555]">{addon.price}</span>
            </li>
          ))}
        </ul>
      </section>
...
function ReviewsSection() {
  return (
    <section id="reviews" className="mt-12 rounded-3xl border border-[#E9E1DA] bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-[#E9E1DA] pb-6 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-[#3B2F2A]">دیدگاه مشتریان</h2>
          <p className="mt-1 text-sm text-[#5A4A40]">۱۲۶ نظر ثبت شده و میانگین امتیاز ۴.۷ از ۵</p>
        </div>
      ...
      <div className="mt-6 space-y-4">
        {reviewData.map((review) => (
          <article key={review.name} className="rounded-3xl border border-[#E9E1DA] bg-[#F9F3ED] p-5">
            <header className="flex flex-wrap items-center gap-3 text-sm text-[#5A4A40]">
              <span className="font-semibold text-[#3B2F2A]">{review.name}</span>
            ...
            <h3 className="mt-3 text-base font-semibold text-[#3B2F2A]">{review.title}</h3>
            <p className="mt-2 text-sm leading-7 text-[#5A4A40]">{review.comment}</p>
            <ul className="mt-4 flex flex-wrap gap-2">
              {review.highlights.map((item) => (
                <li key={item} className="rounded-full bg-white px-3 py-1 text-xs text-[#C78555]">
                  #{item}
                </li>
              ))}
            </ul>
